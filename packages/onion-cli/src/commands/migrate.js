...
let coreDetector, coreConfig, generatorStructure, migrator;
...
async function loadES6Modules() {
  if (!coreDetector) {
    ...
    migrator = await import('../migrator/index.js');
  }
}
...
async function migrate(options = {}) {
  try {
    ...
    const v3 = await coreDetector.detectOnionV3Structure(projectRoot);
    ...
    const eligibility = coreDetector.validateMigrationEligibility(v3);
    ...
    for (const mapping of plan.agents) {
      const targetPath = path.join(projectRoot, mapping.newPath);
      await migrator.transformAgentFile(
        mapping.oldPath,
        targetPath,
        mapping.context
      );
    ...
    if (analysis.stats.hasSessions) {
      ...
      const copied = await migrator.copySessions(projectRoot, 'technical');
